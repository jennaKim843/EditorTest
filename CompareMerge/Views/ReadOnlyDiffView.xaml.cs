...
//                // Modified 체크 완화
//                if (lp == null || rp == null) continue;
//                bool hasBothText = !string.IsNullOrEmpty(lp.Text) && !string.IsNullOrEmpty(rp.Text);
//                if (!hasBothText) continue; // 둘 다 텍스트가 있을 때만 인라인 비교
...
namespace InnoPVManagementSystem.Modules.CompareMerge.Views
{
    public partial class ReadOnlyDiffView : UserControl
    {
        ...
        // 마커 서비스
        private ITextMarkerService? _leftMarkers;
        private ITextMarkerService? _rightMarkers;

        // 라인-컬럼 변경 네비게이션 목록: (editorSide, line, columnIndex)
        private readonly List<(string side, int line, int col)> _nav = new();
        ...
        // 내부 공용 비교 루틴 (LoadFiles에서도 이걸 호출)
        private void CompareCore(string leftText, string rightText)
        {
            var differ = new DiffPlex.Differ();

            // DiffPlex 버전에 따라 안전하게 생성자 사용 (간단버전)
            var sideBuilder = new DiffPlex.DiffBuilder.SideBySideDiffBuilder(differ);
            var side = sideBuilder.BuildDiffModel(leftText, rightText);

            // 문서 세팅 (CompareNow는 이미 텍스트가 들어있으니 생략 가능하지만 안전하게 동기화)
            LeftEditor.Document = new ICSharpCode.AvalonEdit.Document.TextDocument(leftText);
            RightEditor.Document = new ICSharpCode.AvalonEdit.Document.TextDocument(rightText);

            // 마커 준비/정리/적용 (당신이 구현한 메서드 사용)
            EnsureMarkerServices();
            _leftMarkers!.RemoveAll();
            _rightMarkers!.RemoveAll();
            _nav.Clear(); _navIndex = -1;

            var (lcdLeft, lcdRight) = BuildLineColumnDiffs(side);
            ApplyColumnMarkers(LeftEditor, _leftMarkers!, lcdLeft, isLeft: true);
            ApplyColumnMarkers(RightEditor, _rightMarkers!, lcdRight, isLeft: false);

            SetStatus($"행 변경: {CountModifiedLines(side)}개 / 컬럼 변경 네비 항목: {_nav.Count}개");
        }

        // LoadFiles도 내부적으로 CompareCore를 호출하도록 정리해두면 깔끔
        public void LoadFiles(string leftPath, string rightPath)
        {
            var leftText = System.IO.File.ReadAllText(leftPath);
            var rightText = System.IO.File.ReadAllText(rightPath);
            CompareCore(leftText, rightText);
        }
        ...
        //    string leftText = System.IO.File.ReadAllText(leftPath);
        //    string rightText = System.IO.File.ReadAllText(rightPath);

        //    // 1) 라인 비교 (단어/문자 모드 선택 가능)
        //    var differ = new Differ();

        //    var sideBuilder = new SideBySideDiffBuilder(differ, new[] { '\r', '\n' }); // char[] 오버로드
        //    var side = sideBuilder.BuildDiffModel(leftText, rightText);



        //    // 2) 에디터 문서 세팅
        //    LeftEditor.Document = new TextDocument(leftText);
        //    RightEditor.Document = new TextDocument(rightText);
        ...
        private void EnsureMarkerServices()
        {
            _leftMarkers ??= TextMarkerService.Attach(LeftEditor);
            _rightMarkers ??= TextMarkerService.Attach(RightEditor);
        }

        private static int CountModifiedLines(SideBySideDiffModel side)
        {
            int count = 0;
            var leftLines = side.OldText.Lines;
            var rightLines = side.NewText.Lines;
            int n = Math.Max(leftLines.Count, rightLines.Count);
            for (int i = 0; i < n; i++)
            {
                var l = (i < leftLines.Count) ? leftLines[i] : new DiffPiece();
                var r = (i < rightLines.Count) ? rightLines[i] : new DiffPiece();
                if (l.Type == ChangeType.Modified || r.Type == ChangeType.Modified)
                    count++;
            }
            return count;
        }

        private (List<LineColumnDiff> left, List<LineColumnDiff> right) BuildLineColumnDiffs(SideBySideDiffModel side)
        {
            var resLeft = new List<LineColumnDiff>();
            var resRight = new List<LineColumnDiff>();
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                if (l.Type == ChangeType.Modified || r.Type == ChangeType.Modified)
                {
                    var lcd = DiffColumnBuilder.BuildLineColumnDiff(l, r, Delim);
                    if (lcd.Columns.Any(c => c.ChangeType is ChangeType.Modified or ChangeType.Inserted or ChangeType.Deleted))
                    {
                        resLeft.Add(lcd);
                        resRight.Add(lcd);
                    }
        ...
        private void ApplyColumnMarkers(TextEditor editor, ITextMarkerService markers, IList<LineColumnDiff> diffs, bool isLeft)
        {
            foreach (var diff in diffs)
            {
                int line = isLeft ? diff.LeftLineNumber : diff.RightLineNumber;
                ...
                // 컬럼 스팬 재계산 (문서의 실제 텍스트 기준)
                var spans = DiffColumnBuilder.BuildColumnSpans(text, Delim);

                foreach (var c in diff.Columns)
                {
                    var span = isLeft ? c.LeftSpanInLine : c.RightSpanInLine;
                    if (span is null) continue;

                    // 안전 보정
                    int localStart = span.Value.start;
                    int localLength = Math.Min(span.Value.length, Math.Max(0, text.Length - localStart));
                    if (localStart < 0 || localStart >= text.Length) continue;
                    if (localLength <= 0) localLength = 1;

                    int absStart = lineStart + localStart;

                    // 마커 생성
                    var m = markers.Create(absStart, localLength);
                    switch (c.ChangeType)
                    {
                        case ChangeType.Modified:
                            m.BackgroundColor = Colors.LightGoldenrodYellow;
                            m.MarkerType = TextMarkerType.SquigglyUnderline;
                            m.MarkerColor = Colors.DarkGoldenrod;
                            _nav.Add((isLeft ? "L" : "R", line, c.ColumnIndex));
                            break;

                        case ChangeType.Inserted:
                            m.BackgroundColor = Colors.LightGreen;
                            m.MarkerType = TextMarkerType.NormalUnderline;
                            m.MarkerColor = Colors.Green;
                            _nav.Add((isLeft ? "L" : "R", line, c.ColumnIndex));
                            break;

                        case ChangeType.Deleted:
                            m.BackgroundColor = Colors.LightCoral;
                            m.MarkerType = TextMarkerType.DottedUnderline;
                            m.MarkerColor = Colors.DarkRed;
                            _nav.Add((isLeft ? "L" : "R", line, c.ColumnIndex));
                            break;
        ...
        {
            // ViewModel을 안 쓰는 간단 상태 표시 예시
            // XAML 상단 TextBlock 바인딩을 쓰고 있다면 그 바인딩 대상 프로퍼티에 세팅하세요.
            // 여기서는 편의상 직접 접근 예시만 둡니다.
            // DataContext가 ViewModel이면 Status 프로퍼티 업데이트로 대체.
        }
        ...
        private void NavigateTo(int delta)
        {
            ...
            _navIndex = (_navIndex + delta + _nav.Count) % _nav.Count;
            var (side, line, col) = _nav[_navIndex];

            var editor = (side == "L") ? LeftEditor : RightEditor;
            if (line <= 0 || line > editor.Document.LineCount) return;

            editor.ScrollTo(line, 1);
            var docLine = editor.Document.GetLineByNumber(line);
            editor.TextArea.Caret.Offset = docLine.Offset; // 컬럼 시작으로 더 정밀 이동하려면 span.Start 더하기
            editor.TextArea.Caret.BringCaretToView();
        }

        private void OnPrevChange(object sender, RoutedEventArgs e) => NavigateTo(-1);
        private void OnNextChange(object sender, RoutedEventArgs e) => NavigateTo(+1);


        // 현재 Left/RightEditor의 텍스트로 즉시 비교
        public void CompareNow()
        {
            var leftText = LeftEditor?.Text ?? string.Empty;
            var rightText = RightEditor?.Text ?? string.Empty;
            CompareCore(leftText, rightText);
        }
    }
